...
void * Th_Bandera_Function (void * pTh_Args)
{
 ...
 printf("I am thread %d doing %d rows (%d,%d) and %d columns (%d,%d).\n", 
        pMyData->ThreadId, (pMyData->Endi-pMyData->Starti),pMyData->Starti, pMyData->Endi , pMyData->Cols, pMyData->Startj, pMyData->Endj);
 #endif
  ...
  for (int t=0; t<NThreads; t++) 
      {
       ...
       //Balance Static.
       //Rellenamos todos los datos de la estructura para cada hilo
       pTh_Args[t].Rows= Rows;
       pTh_Args[t].Cols= Cols; 
       pTh_Args[t].ppRed= ppRed;
       pTh_Args[t].ppBlue= ppBlue; 
       pTh_Args[t].ppGreen= ppGreen; 
       //Ahora, asignaremos toda la fila completa a cada hilo. Así como un número de filas consecutivo
       pTh_Args[t].Starti= (Rows/NThreads)*t;
       pTh_Args[t].Endi= ((Rows/NThreads)*t) + Rows/NThreads; 
       pTh_Args[t].Startj= 0;
