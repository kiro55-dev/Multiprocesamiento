...
void * Th_Mandel (void * pTh_Args)
{
 ...
 int MyRow=0; 
 int MyChunckSize=*pMyData->pChunckSize; //Rows may be not multiple of NThreads.
 ...
 while (*(pMyData->pRowToProcess)< Rows)
    ...
    if (*(pMyData->pRowToProcess) < pMyData->Rows)
      ...
      //MyChunckSize = pMyData->pChunckSize; -> Alternativa incluirlo en cada iteración
      if(MyRow+MyChunckSize >= Rows) {
         MyChunckSize = Rows-MyRow; 
      }
...
rc = pthread_create(&pThreads[0], NULL, Th_Mandel, (void *)&pTh_Args[0]);
 ...
 if (GenImage)
    { 
	 ...
	 //rawtopgmMain also work using pTh_Args[0]. and pnmtopng are in netpbm package
