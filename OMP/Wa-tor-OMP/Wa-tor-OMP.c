...
#ifdef _OPENMP
   int nthreads = omp_get_max_threads(); // fuera de paralelo
   pRandData = malloc(nthreads * sizeof(struct drand48_data));

#pragma omp parallel
   {
      int tid = omp_get_thread_num();
      long int seed = time(NULL) + tid * 1234; // semilla por hilo
      srand48_r(seed, &pRandData[tid]);
   }
