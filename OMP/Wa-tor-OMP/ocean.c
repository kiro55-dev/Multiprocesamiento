...
#ifdef _OPENMP
        long int randval;
        lrand48_r(&pRandData[omp_get_thread_num()], &randval);
        FreeXYIndex = randval % NFree;
#endif
...
//  to get a random number with lrand48_r() instead of lrand48()
            ...
            long int randval;
            lrand48_r(&pRandData[omp_get_thread_num()], &randval);
            FishXYIndex = randval % NFoundFishes;
#endif
// TODO #ifndef _OPENMP use lrand48()
#ifndef _OPENMP
                  ...
                  const int SiEnergy, const int SeFEnergy, struct drand48_data *pRandData)
{
    ...
    // En vez de dejar espacios de tres en tres solo por filas, tambien se ha hecho por columnas, con el objetivo de que no
    // de la sensación de que se genera de izquierda a derecha, sino la matriz completa simultaneamente.
    for (int offset_i = 0; offset_i < 3; offset_i++)
    {
        for (int offset_j = 0; offset_j < 3; offset_j++)
        {
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) reduction(+ : localNFishes, localNSharks)
#endif
            for (int i = offset_i; i < Rows; i += 3)
            {
                for (int j = offset_j; j < Cols; j += 3)
                {
                    if (Ocean[i][j] != NULL && Ocean[i][j]->Animal == FISH)
                        IterateFish(Ocean, Rows, Cols, i, j, SimIter, &localNFishes, NiFBreed, pRandData);
                    if (Ocean[i][j] != NULL && Ocean[i][j]->Animal == SHARK)
                        IterateShark(Ocean, Rows, Cols, i, j, SimIter,
                                     &localNFishes, &localNSharks, NiSBreed, SiEnergy, SeFEnergy, pRandData);
                }
            }
        }
    }
    *pNFishes += localNFishes;
    *pNSharks += localNSharks;
}
